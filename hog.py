...
def is_swap(player_score, opponent_score):
    ...
    return opponent_ten == abs(player_one - opponent_one)
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    prev = [0, 0] #previous scores of both players
    while scores[0] < goal and scores[1] < goal:
        rolls = strategies[who](scores[who], scores[other(who)])
        points = take_turn(rolls, scores[other(who)], dice)
        scores[who] += points
        if feral_hogs == True:
            if abs(rolls - prev[who]) == 2:
                scores[who] += 3
        prev[who] = points
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def helper(s0, s1):
        scr_lst = [s0, s1]
        earned = scr_lst[who] - last_score
        if earned > running_high:
            print(earned, "point(s)! That's the biggest gain yet for Player", who)
            return announce_highest(who, scr_lst[who], earned)
        return announce_highest(who, scr_lst[who], running_high)
    return helper
    # END PROBLEM 7
